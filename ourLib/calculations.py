...
def mean_opperation(Nifti_file_collection,file_name):    
    ...
    print('\nMean process was successfull ! File result : \''+file_name+'\'')
    return file_Nifti_clusterised


def or_opperation(Nifti_file_collection,file_name): 
    (lx,ly,lz) = max_shape(Nifti_file_collection)
    file_Nifti_clusterised = np.zeros(shape=(lx,ly,lz), dtype='f')
    for file in Nifti_file_collection:
        list_voxels = Extract_voxels_from_Nifti_file(file)
    ...
    img = nib.Nifti1Image(file_Nifti_clusterised, np.eye(4))
    nib.save(img,file_name)
    print('\nOr opperation process was successfull ! File result : \''+file_name+'\'')
    return file_Nifti_clusterised
...
def and_opperation(Nifti_file_collection,file_name): 
    (lx,ly,lz) = max_shape(Nifti_file_collection)
    file_Nifti_clusterised = np.zeros(shape=(lx,ly,lz), dtype='f')
    list_pix = []
    for file in Nifti_file_collection:
    ...
    print('\nAnd opperation process was successfull ! File result : \''+file_name+'\'')
    return file_Nifti_clusterised

def linear_combination_opperation(Nifti_file_collection,coef,file_name):
...
def normalization_opperation(Nifti_file_collection):
    ...
    for file in Nifti_file_collection:
        ...
        for voxels in list_voxels:
            ...
            y = voxels[1]
            z = voxels[2]
            file_Nifti_clusterised[x][y][z] = data[x][y][z]/float(max)
        #On enregistre le cluster dans un nouveau fichier Nifti 
        img = nib.Nifti1Image(file_Nifti_clusterised, np.eye(4))
        nib.save(img,file)
    print('\nNormalized process with file(s) was successfull !')
    return file_Nifti_clusterised
